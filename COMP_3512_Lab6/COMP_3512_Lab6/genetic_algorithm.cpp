...
int main(void)
{
	...
	/* Initializes static allocated memory to specified sizes */
	...
	for (i = 0; i < POPULATION_SIZE; ++i) {
		//(population + i)->permutation.resize(permutation);
		for (j = 0; j < CITIES_IN_TOUR; ++j) {

			// Copies the master list of city structs to each tour...
			tempPermutation.push_back(*(cities_to_visit));
		}

		// ...and then randomly shuffles them.
		tempPermutation = shuffle_cities(tempPermutation); // Randomizes order of cities in this tour
		...
		while(!tempPermutation.empty())
		{
			(population + i)->permutation.push(*tempPermutation.begin());
			tempPermutation.erase(tempPermutation.begin());
		}
...
struct tour * crossover(struct tour * parents)
{
	...
	for (i = 0; i < boundary_index; i++) {
		...
		//child->permutation.push(*(parents->permutation[i]));
	}
...
* RETURN: VOID
*/
vector<city> shuffle_cities(struct vector<city> permutation)
{
...
*/
vector<city> swap_cities(int index_one, int index_two, struct vector<city>  permutation)
{
	city* temp = (struct city *) malloc(sizeof(struct city));
